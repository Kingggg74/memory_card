...
q2 = Question('В каком году написана первая статья про Python?' , '1980', '2000', '2023', '2015')
q3 = Question('Может ли быть индекс списка отрицательным?' , 'да', 'нет', 'в определеных случаях', 'иногда')
q4 = Question('Python полностью поддерживает ООП?' , 'да', 'нет', 'частично', 'раньше подерживали')
q5 = Question('Что может быть ключом в словаре?' , 'любое слово', 'неизменяемое слово', 'переменая', 'ничего')
q6 = Question('Каким будет результат следующего выражения >>> [0, 1][10:]' , 'пустой список', '5', '10', '1')
q7 = Question('аким будет результат следующего выражения >>> len(' '.join(list(map(str, [[0], [1]]))))' , '3', '2', '4', '5')
q8 = Question('Поддерживает ли Python множественное наследование' , 'да', 'нет', 'иногда', 'в определеных случаев')
q9 = Question('С помощью какого инструмента можно выполнить статический анализ кода' , 'Pylint', 'global', 'def', 'class')
q10 = Question('Кто использует термин "Черная дыра"?' , 'Астрономы', 'Стоматологи', 'Портные', 'Сантехники')
question_list = list()
...
desktop = app.desktop()
main_win = QWidget()
main_win.setStyleSheet("background-image: url(pgi.webp)")
main_win.resize(int(desktop.width() * 0.65), int(desktop.height() * 0.65))
